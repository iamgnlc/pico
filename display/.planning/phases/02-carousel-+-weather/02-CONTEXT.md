...
- **D-23** *(RETIRED 2026-07-18 — see `.planning/quick/260718-remove-spinner-tune-cadence/` for the retirement audit trail. The static `connecting...` text during `wifi.connect()` remains; the spinner during weather HTTP fetch was removed at operator request. Weather-view refresh now runs silently — the panel keeps showing the last-known cache state during background fetches. Original text preserved below for historical reference.)*:
