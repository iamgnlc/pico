...
## Notes (added at milestone archive)

### WEATHER-08 dropped mid-milestone

WEATHER-08 ("The Weather view displays the current location name") was originally scoped into Phase 2.1 and shipped as Plan 02.1-01. During on-device verification, the operator caught a layout regression (the location label pushed the temperature down, and the visual balance felt worse than the Phase 2 baseline). The plan was reverted via `/gsd:undo` (commit `b8823ab`), and WEATHER-08 was formally dropped from v1 via quick task `260718-scope-cleanup-phase-2.1`. Phase 2.1 narrowed to retry-only (WEATHER-09).

The device does now fetch location data via ip-api on every weather cycle (piggybacked with tz_offset and WAN IP) — but only WAN IP is surfaced in the UI (System view, per SYSTEM-02 as redirected by Phase 4 D-43-bis). A future v2 might resurrect a location-visible feature under a different layout.

### CLOCK-02 wording evolved mid-phase

CLOCK-02 was originally worded as "single hardcoded timezone offset in main.py". Phase 3 shipped Plan 03-01 with that exact behavior. On-device human-verify then flagged the manual-config brittleness (the operator would have to edit main.py for every timezone, DST transition, or move). Plan 03-02 (gap closure within Phase 3) redirected CLOCK-02 to "auto-derived from ip-api's `offset` field, persisted to `tz_offset.txt`". The requirement wording above reflects the delivered (redirected) form, not the original.

### Traceability status Complete = code + human-verify passed

For every v1 requirement listed above with `[x]` and status "Complete":

- Code shipped in a `feat(...)` commit under the corresponding phase's plan.
- Automated grep + syntax + anti-diff verification passed at commit time.
- On-device human-verify was executed and approved by the operator before the phase's VERIFICATION.md was marked PASSED.

For the exact evidence per requirement, see the phase-level VERIFICATION.md files (`01-VERIFICATION.md`, `02-VERIFICATION.md`, `02.1-VERIFICATION.md`, `03-VERIFICATION.md`, `04-VERIFICATION.md`) — frozen alongside their phase directories inside `.planning/phases/`.

---

_This archive is frozen. For v2 planning, see the fresh `.planning/REQUIREMENTS.md` created by `/gsd:new-milestone`._
